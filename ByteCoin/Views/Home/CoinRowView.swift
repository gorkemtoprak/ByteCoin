...
struct CoinRowView_Previews: PreviewProvider {
    static var previews: some View {
        Group{
            ...
            CoinRowView(coin: dev.coin, showHoldingColumn: true).previewLayout(.sizeThatFits).preferredColorScheme(.dark)
        }
    }
}
...
extension CoinRowView {
    ...
    private var rightColumn: some View{
        VStack(alignment: .trailing){
            Text(coin.currentPrice.asCurrencyWith6Decimals()).bold().foregroundColor(Color.theme.accent)
